...
export interface OpenAIConceptImageResult {
  base64: string;
...
function parseImageSize(size?: string | null): { width?: number; height?: number } {
...
export async function generateConceptImageWithOpenAI(prompt: string): Promise<OpenAIConceptImageResult> {
  requireServerEnv('OPENAI_API_KEY');
...
export async function generateCurriculumWithOpenAI(goal: string, subject: string, difficulty: string): Promise<any> {
  ...
  const response = await generateWithOpenAI(prompt, { temperature: 0.6, maxTokens: 12000 });
