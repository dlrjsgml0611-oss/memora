...
export async function generateWithClaude(prompt: string, options?: {
  ...
  requireServerEnv('ANTHROPIC_API_KEY');
  try {
...
export async function generateCurriculumWithClaude(goal: string, subject: string, difficulty: string): Promise<any> {
  ...
  const response = await generateWithClaude(prompt, { temperature: 0.6, maxTokens: 8000 });
...
export async function generateConceptWithClaude(
  topicTitle: string,
...
export async function generateMindmapWithClaude(
  conceptTitle: string,
...
마인드맵 구조:`;
  ...
  const response = await generateWithClaude(prompt, { temperature: 0.7, maxTokens: 3000 });
...
export async function generateMemoryPalaceWithClaude(
  conceptTitle: string,
  conceptContent: string
): Promise<any> {
...
export async function generateMnemonicWithClaude(
  subject: string,
  technique: string,
  content: string
): Promise<string> {
...
기억술:`;
  ...
  return generateWithClaude(prompt, { temperature: 0.8, maxTokens: 2000 });
}
